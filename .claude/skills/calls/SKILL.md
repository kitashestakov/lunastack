...
- **API shape variations**: `tldv.sh` пробует несколько имён полей (`results / data / items`, `invitees / participants / attendees`). Если tldv поменяет API и фильтр перестанет работать — список будет пустым, не будет утечки чужих звонков. Это safe-by-default.
