...
2. **No em-dashes.** Никаких `—` (U+2014) и `–` (U+2013). Только обычный дефис `-` (U+002D) и то только там, где грамматически нужен (составные слова, диапазоны). Em-dash — это #1 AI-маркер в русском, модели его обожают, поэтому выжигаем каленым железом.

   **Как заменять em-dash в зависимости от контекста (НЕ механическая замена на `-`):**
   - «Мы работаем с отелями — и с ресторанами» → «Мы работаем с отелями, и с ресторанами» (запятая)
   - «Цена €5K — все включено» → «Цена €5K, все включено» или «€5K. Все включено» (запятая или точка)
   - «Два открытия — зеро персонала — дедлайн» → «Два открытия, ноль персонала, дедлайн» (запятые)
   - «Настя — founder Luna Pastel» → «Настя, founder Luna Pastel» (запятая)
   - «Следующий шаг — созвон» → «Следующий шаг: созвон» или «Давай созвонимся» (двоеточие или переформулировка)
   - В диалоге-реплике «— Привет!» → убрать тире, просто «Привет!»

   Если после замены предложение звучит коряво — переформулируй целиком, а не пытайся вставить другой знак.
...
| `ё` | AI-маркер, Настя никогда не пишет ё | Замени на `е` (кроме `всё`/`все` где смысл меняется) |
...
| Три `)` подряд или больше | Слишком неформально | Максимум `))`, и не в каждом сообщении |
| Более одного emoji | Превышение квоты | Оставь один самый уместный, остальные удали |
