...
Use AskUserQuestion:
- question: «На каком этапе брифинга?»
- header: «Этап»
- options:
  - label: "Подготовка к брифингу (Рекомендуется)"
    description: "Сгенерирую вопросы и шаблон скоринг-таблицы — заполнить в Notion перед встречей с клиентом"
  - label: "Брифинг прошёл, сохранить транскрибт"
    description: "Подтяну запись звонка из tldv, сохраню транскрибт в страницу «Транскрибт брифинга»"

If «Подготовка» → Step 1.
If «Брифинг прошёл» → skip to Step 6.
...
- «После брифинга вернись в эту сессию и снова набери `/briefing` — выберешь "Брифинг прошёл, сохранить транскрибт". Я подтяну запись из tldv и положу в "Транскрибт брифинга". Скоринг-таблицу при необходимости заполни руками. Могу помочь с /research для углубленного анализа или /outreach для первых сообщений кандидатам.»

## Step 6: Save briefing transcript from tldv

Этот шаг выполняется когда в Step 0 рекрутер выбрал «Брифинг прошёл, сохранить транскрибт».

Делегируй работу скиллу `/calls` — у него уже есть весь flow: token migration, фильтр по email рекрутера, выбор звонка, сохранение в правильную sub-page вакансии.

**Не дублируй логику /calls здесь.** Просто:

1. Прочитай `.claude/skills/calls/SKILL.md` целиком и следуй его инструкциям, начиная с Pre-check 0 (token + email migration), затем сразу перейди в его Step 6 (Save transcript to vacancy in Notion).
2. Параметры контекста для /calls:
   - **Текущая вакансия** уже привязана к session — используй её, не запрашивай повторно
   - **Тип транскрибта** — сразу «Брифинг» (не показывай AskUserQuestion из Step 6b /calls), целевая sub-page — «Транскрибт брифинга»
3. После того как /calls завершит запись — подтверди: «Транскрибт брифинга сохранён в карточку вакансии. Дальше можешь заполнить «Скоринг-таблицу» руками или запустить /vacancy-card для оформления описания позиции.»

Если в tldv нет подходящего звонка (не найден или у tldv API нет доступа) — сообщи об этом и предложи рекрутеру вставить транскрибт текстом вручную в страницу «Транскрибт брифинга» (через UI Notion, не через скилл).
